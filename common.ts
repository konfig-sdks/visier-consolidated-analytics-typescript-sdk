...
import { readableStreamToString, VisierConsolidatedAnalyticsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierConsolidatedAnalyticsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierConsolidatedAnalyticsError(e, e.response?.data)
                }
                if (innerError instanceof VisierConsolidatedAnalyticsError) {
                    // Got "VisierConsolidatedAnalyticsError" from the above try block
                    throw innerError;
                }
